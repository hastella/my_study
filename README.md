...
예를 들어, 계단을 오르는데 한 번에 1칸 또는 2칸씩 올라갈 수 있다면, 10번째 계단에 오르는 방법의 수는? 이런 문제를 풀 때 매번 똑같은 계산을 반복하지 않고, **한 번 계산한 결과를 저장해두고 재사용**하는 방법이 DP다!

**언제 DP를 사용할까?**
- 같은 계산이 반복되는 문제
- 작은 문제의 답을 조합해서 큰 문제를 풀 수 있는 경우

**구현 방법**
- **Top-down**: 큰 문제부터 시작해서 작은 문제로 쪼개며 내려감 (재귀 사용)
- **Bottom-up**: 작은 문제부터 풀어서 큰 문제로 올라감 (반복문 사용)

🤓 요약: DP는 중복 계산을 피하기 위해 계산 결과를 저장해두고 재사용하는 방법. 코딩테스트에서 피보나치, 계단 오르기, 배낭 문제 등에서 자주 사용된다!
<br>
